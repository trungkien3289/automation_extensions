export class ScreenMask{
    static init(){
		console.log("fake screen");
        chrome.storage.local.get('config', (storage) => {
			const config = storage.config;
			let isMobile = ScreenMask.mobileCheck(config.userAgent);
			console.log("config", config);
			console.log("is mobile:", isMobile);
			if(config && config.screen != null && isMobile){
				const embedScript = () => {
					const code = `(() => {
	...
	static mobileCheck = function(userAgent: string) {
		let check = false;
